...
CGameEngineWin32::CGameEngineWin32( HINSTANCE hInstance, int nShowCommand, int32 nWindowWidth, int32 nWindowHeight )
{
	...
	if ( !BInitializeD3D9() )
	{
		::MessageBoxA( NULL, "Failed to initialize D3D9.\n\nGame will now exit.", "Spacewar - Fatal error", MB_OK | MB_ICONERROR );
		return;
	}
	...
	{
		::MessageBoxA( NULL, "Failed to init XAudio2 engine (grab the latest \"DirectX End-User Runtime Web Installer\" )", "Spacewar - Fatal error", MB_OK | MB_ICONERROR );
		return;
	}
	...
	{
		::MessageBoxA( NULL, "Failed to create mastering voice", "Spacewar", MB_OK | MB_ICONERROR );
	}
...
void CGameEngineWin32::Shutdown()
{
	...
	if ( m_hInstance )
	{
		if ( !UnregisterClass( "Spacewar", m_hInstance ) )
		{
